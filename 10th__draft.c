...
Matrix init_mult_matrix(Matrix * matA, Matrix * matB)
{
    ...
    {
        for ( row_idx = 0; row_idx < matC.row_size; row_idx++ )
        {
    ...
    else
    {
        ...
        while (col_idx < matC.col_size)
        {
            ...
            while (row_idx < matC.row_size)
            {
                ...
                fflush(stdout);
                matC.A[matA->row_size * col_idx + row_idx] = matA->A[matA->row_size * col_idx + row_idx] + matB->A[matB->row_size * col_idx + row_idx];
                printf(" IJ of A %lf\n", matA->A[matA->row_size * col_idx + row_idx]);
                printf(" IJ of B %lf\n", matB->A[matA->row_size * col_idx + row_idx]);
                row_idx++;   
            }
    ...
    else
    {
        ...
        for ( int col_idx = 0; col_idx < matC.col_size; col_idx++ )
        {
            ...
            for ( int row_idx = 0; row_idx < matC.row_size; row_idx++ )
            {
                ...
                {
                    matC.A[matA->row_size * col_idx + row_idx] +=  matA->A[matA->row_size * k + row_idx] +  matB->A[matB->row_size * col_idx + k];
