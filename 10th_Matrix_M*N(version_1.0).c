...
Matrix dynamic_array_alloc(int row_size, int col_size)
{
    Matrix matA;
...
Matrix init_matrix(Matrix * matA)
{
    ...
    int col_idx = 0;
    int row_idx = 0;
    matB.row_size = matA->row_size;
    matB.col_size = matA->col_size;
    for ( row_idx = 0; row_idx < matA->row_size; row_idx++ )
    {
        for ( col_idx = 0; col_idx < matA->col_size; col_idx++ )
        {
            matB.A[matA->row_size * row_idx + col_idx] = 0;
        }
    }
    return matB;
}
...
Matrix make_diagonal_matrix( Matrix  *matA )
{
    ...
    for ( row_idx = 0; row_idx < matA->row_size; row_idx++ )
    {
        for ( col_idx = 0; col_idx < matA->col_size; col_idx++ )
        {
            matB.A[matA->row_size * row_idx + col_idx] = (int)(col_idx != row_idx) * 2;
        }
    }
    return matB;
}
...
void PrintMatrix(Matrix *matA)
{
    ...
    for ( row_idx = 0; row_idx < matA->row_size; row_idx++ )
    {
        for ( col_idx = 0; col_idx < matA->col_size; col_idx++ )
        {
            printf( "%d\t", matB.A[matA->row_size * row_idx + col_idx]);
        }
    printf( "\n" );
    }
    printf("\n");
