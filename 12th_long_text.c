...
char *my_realloc(char *string, int *len, int previous_size)
{
    char *new_string = (char *)malloc(previous_size * 2 *sizeof(char));
    memcpy(string, new_string, len * sizeof(char));
    free(string);
...
char *get_string(int *len) 
{
    *len = 0; 
    ...
    while (c != '#') 
    {
        ...
        if (*len >= start_size)
        {
            string = my_realloc(string, len, start_size);
        }
